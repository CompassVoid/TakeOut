            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                                <el-button v-if="isAuth('caipin','查看')" type="text" icon="el-icon-edit" size="small" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                                                                ...
                                                                <el-button v-if="isAuth('caipin','加入订餐车')" type="text" icon="el-icon-shopping-cart-full" size="small" @click="addToCart(scope.row)">加入购物车</el-button>
...
export default {
  ...
  methods: {
    ...
    addToCart(row){
      const cart = this.$storage.getObj('cartItems') || []
      const found = cart.find(item => item.id === row.id)
      if(found){
        found.goumaishuliang = (found.goumaishuliang || found.quantity || 1) + 1
      }else{
        cart.push({
          id: row.id,
      ...
      this.$storage.set('cartItems', cart)
